(ns basicchem.operation
  "OperationActor -- one basic-chemicals-manufacturing operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Basic Chemicals Advisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Chemical Safety
  ...
  One graph run = one basic-chemicals-manufacturing operation (advise
  ...
  decision to a human operator (the chemical-safety engineer). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:log-production-batch`/`:release-batch`/`:flag-safety-
  concern` ALWAYS reach this node when the governor is clean -- see
  `basicchem.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [basicchem.basicchemadvisor :as basicchemadvisor]
            [basicchem.governor :as governor]
...
(defn build
  ...
  `basicchem.store/Store`).
  opts:
    :advisor      -- a `basicchem.basicchemadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (basicchemadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Basic Chemicals Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (basicchemadvisor/-advise advisor store request)]
            {:proposal p :audit [(basicchemadvisor/trace request p)]})))

      ;; Chemical Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
